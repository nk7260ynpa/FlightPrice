...
**選擇**：四層查詢鏈，依序嘗試直到取得完整航線資訊

**查詢鏈**：
1. **DB 快取**（tracked_flights 表）：同班次已有紀錄時直接複製 airline/origin/destination，零成本
2. **Flightradar24 網頁爬取**（主要來源）：免費公開，從 `flightradar24.com/data/flights/{flight_number}` 爬取完整航線資訊，需模擬瀏覽器 headers 避免被阻擋
3. **AviationStack API**（備援）：需 API Key，有免費額度限制
4. **IATA 代碼對照表**（最後備援）：僅能解析航空公司名稱，出發地/抵達地需手動輸入

**理由**：Flightradar24 免費且包含完整航線資訊（出發地/抵達地 IATA 代碼），搭配 DB 快取可大幅減少外部請求。航線資料幾乎不變，查到一次即可永久快取於 tracked_flights 表。

**替代方案**：Skyscanner（無法以班次代碼查航線）、FlightAware（需登入）、Google Flights（無直接 API）
...
- **[Flightradar24 反爬蟲機制]** → 需設定適當的 User-Agent 與請求間隔，避免被封鎖。若被阻擋則自動降級至 AviationStack API
