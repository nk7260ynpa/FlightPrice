...
- **相依**: 無
- **驗收條件**:
  - [x] 於 `app/__init__.py` import `os`（已存在，沿用即可）。
  - [x] 將原本
  ...
  - [x] 不得修改 `app/scheduler.py`、`app/routes/*`、`app/scraper.py`。
  - [x] 不得修改 `docker/`、`.env`、`requirements.txt`、`run.sh`。
  - [x] 符合 spec `specs/scheduler/spec.md` 中所有 Scenario 描述的行為。
...
- **相依**: Task 1
- **驗收條件**:
  - [x] 新建測試檔 `tests/test_app_factory.py`，使用 `unittest.mock.patch`
    攔截 `app.scheduler.init_scheduler`，驗證其是否被呼叫。
  - [x] 測試禁止真正啟動 `BackgroundScheduler`（必須 mock，不得在測試中建立
    真實排程 thread）。
  - [x] 實作測試 `test_testing_mode_does_not_start_scheduler`：
    對應 spec Scenario「測試模式不啟動 scheduler」。
  - [x] 實作測試 `test_production_mode_starts_scheduler`：
  ...
  - [x] 實作測試 `test_debug_reloader_parent_does_not_start_scheduler`：
    對應 spec Scenario「Debug reloader parent 不啟動 scheduler」；
    `app.debug=True` 且 `WERKZEUG_RUN_MAIN` 未設。
  - [x] 實作測試 `test_debug_reloader_child_starts_scheduler`：
    對應 spec Scenario「Debug reloader child 啟動 scheduler」；
    `app.debug=True` 且 `monkeypatch.setenv('WERKZEUG_RUN_MAIN', 'true')`。
  - [x] 實作測試 `test_debug_with_werkzeug_run_main_false_does_not_start`：
    對應 spec Scenario「WERKZEUG_RUN_MAIN 為其他值時不啟動」。
  - [x] 所有測試於 `docker compose run --rm app pytest tests/test_app_factory.py`
    或 `./run.sh pytest tests/test_app_factory.py` 下通過。
  - [x] 不得修改既有測試檔 `tests/conftest.py`、`tests/test_*.py`。
  - [x] 不得修改 `app/` 任何檔案（實作已於 Task 1 完成）。
...
- **相依**: Task 1, Task 2
- **驗收條件**:
  - [x] 執行 `./run.sh pytest` 或 `docker compose run --rm app pytest`，
    全套測試（含既有 `test_charts.py`、`test_flights.py` 等）通過。
  - [x] 若任一既有測試失敗，於 `issues.md` 記錄並回報 Coordinator，不得
    自行修改超出範圍的檔案。
